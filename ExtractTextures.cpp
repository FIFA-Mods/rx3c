...
void ConvertDDS(path const &texPath, bool toPNG, bool deleteDDS) {
    ...
    if (SUCCEEDED(D3DXCreateTextureFromFileW(globalVars().device->Interface(), texPath.c_str(), &d3dTex))) {
        ...
        destPath.replace_extension(toPNG ? ".png" : ".tga");
        D3DXSaveTextureToFileW(destPath.c_str(), toPNG ? D3DXIFF_PNG : D3DXIFF_TGA, d3dTex, NULL);
        d3dTex->Release();
        if (deleteDDS) {
            error_code ec;
            remove(texPath, ec);
...
void ExtractTexturesFromContainer(Rx3Container &container, path const &outputDir, Rx3Options const &rx3options) {
    ...
    bool saveToTga = false;
    if (rx3options.textureFormat == "png")
        saveToPng = true;
    else if (rx3options.textureFormat == "tga")
        saveToTga = true;
    bool isPngOrTga = saveToPng || saveToTga;
    for (size_t i = 0; i < textureSections.size(); i++) {
        ...
        if (ddsHeader) {
            ...
            if (levels > 0) {
                if (saveToPng || saveToTga)
                    levels = 1;
                bool isCubemap = !isPngOrTga && type == 3 && faces == 6;
                FILE *file = nullptr;
                path texPath;
                bool written = false;
                ...
                if (isPngOrTga && written)
                    ConvertDDS(texPath, saveToPng, true);
            }
        }
    }
}
